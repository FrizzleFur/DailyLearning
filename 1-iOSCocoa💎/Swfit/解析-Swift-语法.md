...
![](https://pic-mike.oss-cn-hongkong.aliyuncs.com/Blog/20200420162500.png)
...
```
这个例子中使用unowned原因是可以保证这个block的生命周期是比DemoController短的（view是DemoController的属性），也就是DemoController释放之后是肯定不都会调用到这个函数的。
```swift
class DemoController: UIViewController {
    func buttonTapped() {
        let controller = NextController { [weak self] in
...
```
相反，上面的例子中，NextController的生命周期是不确定的，是可能比DemoController短，也可能比DemoController长的，这种情况下应该使用weak
...
class DemoController: UIViewController {
    func buttonTapped() {
        let arr = ["1", "2"]
        arr.forEach { [unowned self] in // 错误的例子
...
上面的这个例子中，在非逃逸闭包里有时候也会加unowned和weak修饰，其实是不需要的，因为非逃逸闭包的生命周期是确定的，forEach函数执行完毕之后就可以释放了，不会被其他对象持有，导致生命周期超出buttonTapped的执行范围。
...
class DemoController: UIViewController {
    ...
    init(observable: Observable<String>) {
        ...
        observable.subscribe(onNext: { [unowned self] in // 错误的例子，应该用weak
            self.doSomething()
...
对于这个case，首先需要分析的是为什么onNext回调为什么需要加修饰符，self持有observable，observable会持有onNext回调（这里可以看下subscribe函数的源码），onNext回调又持有self，这样就会形成循环引用。
再来分析用什么修饰符，首先信号订阅是通过该self.disposeBag管理的，DemoController deinit时，先释放self.disposeBag，所以我们会认为onNext回调的生命周期是比self短的，用unowned应该没有问题。**但是这里要注意的是这个observable是从外面传入的，发送信号的时机是不确定的，我们遇到很多次crash都是对象deinit时，正好发射了信号，此时会进到onNext回调导致crash**。

Dispose logic should never perform anything except disposing, like sending elements. The code will execute properly, but semantically it causes undefined behavior.
Rx上有人提了类似的issue，作者也做出过回应，如果在dispose的过程中，发射了数据会引起未定义的行为。所以从安全性上考虑，onNext等回调里最好使用weak修饰。

这里总结了几条原则
1. 非逃逸闭包：不用加修饰符
2. 逃逸闭包：确定生命周期比self短的用unowned，不确定的用weak
3. 信号监听：onNext等回调中统一用weak修饰符，用unowned并不是绝对安全；信号的map、flatMap等操作符也应该是用weak修饰
