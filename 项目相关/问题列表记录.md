...
* [ ] 明星主页：
    productTickLb.text = @"300 万";
    bannerTitleLb.text = @"本期应援目标";

* 豆发布了微博 等文案确认

* /** 回调函数--更新排行榜入口信息 */
- (void)vm_getRankEntryByStarNoCallBack:(PeriodPickInviteModel *)periodPickInviteModel{
    //是否审核中-隐藏违规信息
    BOOL isAuditing = [AppArchiverInstance isAuditing];
    if (isAuditing) return;
    
    // 显示打榜入口
    NSNumber *rankEntryType = @(StarPageModule_DaCall_Hits);
    BOOL hasRankEntry = [self.daCallTypeArr containsObject:rankEntryType];
    // 是否开启
    if (periodPickInviteModel.open) {
        self.periodPickInviteModel = periodPickInviteModel;
        if (!hasRankEntry) {
            // 先回调打榜入口，再插入微信助手入口
            NSNumber *listType = @(StarPageModule_DaCall_CardList);
            NSInteger listIndex = [self.daCallTypeArr indexOfObject:listType];
            [self.daCallTypeArr insertObject:rankEntryType atIndex:listIndex + 1];
        }
    }else{
        if (hasRankEntry) [self.daCallTypeArr removeObject:rankEntryType];
    }
